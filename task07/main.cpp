  ...
  for (unsigned int iw = 0; iw < img_width; ++iw) {
    for (unsigned int ih = 0; ih < img_height; ++ih) {
      ...
      for (int isample = 0; isample < nsample; ++isample) {
        ...
        float rad = hit0_brdf * hit1_rad * std::max(0.0f, hit0_normal.dot(hit0_refl)) / hit0_pdf;
      ...
      for (int isample = 0; isample < nsample / 2; ++isample) {
        ...
        float rad = weight * hit0_brdf * hit1_rad * std::max(0.0f, hit0_normal.dot(hit0_refl)) / hit0_pdf_brdf_sample;
